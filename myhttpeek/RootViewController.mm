...
+(NSMutableArray *)iOS8GetApplist{
    ...
    NSMutableArray *appList = [NSMutableArray arrayWithCapacity:10];
        for(NSString *PathTmp in arrary)
        {
            PathTmp = [path stringByAppendingString:PathTmp];
...
//        NSLog(@"Name = %@",[[self.dataList objectAtIndex:1] appName]);
    ...
    {
        self.dataList = [RootViewController iOS8GetApplist];
...
-(void)clickAudit:(id)sender{
    if (self->global_flag == 1) {//已经选择了APP，等待点击审计
        ...
        [RootViewController DelAndAddAppToPlist:bundle];
    ...
    else if(self->global_flag  == 2)        // 正在审计的状态
    {
        ...
        NSString *Title = [NSString stringWithFormat:@"开始审计 APP流量\n开启应用"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(self->global_flag == 0)//开始选择APP
    {
        ...
        [but setTitle:@"开始审计 APP流量\n开启应用" forState:UIControlStateNormal];
        
        self->global_flag = 1;
    }
    else if(self->global_flag == 1){//已经选择了APP，等待点击审计
        self.global_index = indexPath;
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"请点击开始审计 App 流量" message:nil delegate:nil cancelButtonTitle:@"ok" otherButtonTitles:nil];
        [alert show];
    }
    else if(self->global_flag == 2)// 正在审计的状态
    {
        if (indexPath.row == self.global_index.row)
        {
           
            NSString *msg = [[NSString alloc] initWithData:self->postData encoding:NSUTF8StringEncoding];
            /*
              NSLog(@"add view start");
            CGRect frametmp = self.view.frame;
            frametmp.origin.y += 50;
            frametmp.size.height -= 70;
            
            NSTextContainer *textcontainertmp = [[NSTextContainer alloc] initWithSize:frametmp.size];
            
            UITextView *textviewtmp = [[UITextView alloc] initWithFrame:frametmp textContainer:textcontainertmp];
            
            textviewtmp.textColor = [UIColor redColor];
            textviewtmp.text =msg;
            
            UIButton *buttmp = [UIButton buttonWithType:UIButtonTypeCustom];
            buttmp.frame = CGRectMake(0,self.view.frame.size.height, self.view.frame.size.width,50);
            [buttmp setTitle:@"回来前页面" forState:UIControlStateNormal];
            buttmp.backgroundColor = [UIColor colorWithWhite:0.648 alpha:1.000];
            [buttmp.titleLabel setLineBreakMode:NSLineBreakByCharWrapping];
            [buttmp addTarget:self action:@selector(clickremoveview:) forControlEvents:UIControlEventTouchUpInside];
            
            viewtmp = [[UIView alloc] initWithFrame:self.view.frame];
             [viewtmp addSubview:buttmp];
